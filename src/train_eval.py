...
def main():
    ...
    phase = "Multispectral"
    if train_conf.wandb_enable:
        wandb.init(project="hjkim_mlpd_attention", entity="hjkim_jun")
...
def train_epoch(model: SSD300,
    ...
    for batch_idx, (image_vis, image_lwir, boxes, labels, _) in enumerate(dataloader):
        ...
        if config.train.wandb_enable:
            wandb.log({"loss": loss})
            wandb.log({"cls_loss": cls_loss})
            wandb.log({"loc_loss": loc_loss})
