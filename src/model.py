...
class MCFF(nn.Module):
    def __init__(self, RGB_features=None, LWIR_features=None, ch_dim = 512):
        super().__init__()
        self.RGB_features = RGB_features
        self.LWIR_features = LWIR_features
        # self.conv_RGB_feats = nn.Conv2d(512, 1024, kernel_size=3, padding=1, bias=True) 
        # self.conv_LWIR_feats = nn.Conv2d(512, 1024, kernel_size=3, padding=1, bias=True) 

        self.gap = nn.AdaptiveAvgPool2d(1)
        self.gmp = nn.AdaptiveMaxPool2d(1)
        self.fc1 = nn.Linear(ch_dim*2, ch_dim, bias= True)
        self.fc2 = nn.Linear(ch_dim*2, ch_dim, bias= True)
        nn.init.xavier_uniform_(self.fc1.weight)
        nn.init.xavier_uniform_(self.fc2.weight)

    def forward(self, RGB_features, LWIR_features):
        # torch.Size([2, 256, 76, 76])  
        assert RGB_features!=None or LWIR_features!=None
        _, c, _, _ = RGB_features.shape

        out = torch.cat([RGB_features, LWIR_features], dim=1) # torch.Size([2, 1024, 76, 76])

        out = self.gap(out) + self.gmp(out) # torch.Size([2, 1024, 1, 1])
        out = out.view(len(out), -1)

        out_RGB = self.fc1(out)
        out_lwir = self.fc2(out)
        
        out_RGB = nn.functional.softmax(out_RGB, dim=1)
        out_lwir = nn.functional.softmax(out_lwir, dim=1)
        
        # alpha = out[:,:c].unsqueeze(dim=-1).unsqueeze(dim=-1)
        # beta = out[:,c:].unsqueeze(dim=-1).unsqueeze(dim=-1)
        alpha = out_RGB.unsqueeze(dim=-1).unsqueeze(dim=-1)
        beta = out_lwir.unsqueeze(dim=-1).unsqueeze(dim=-1)
        return torch.cat([alpha*RGB_features, beta*LWIR_features], dim=1)

class MCFF2(nn.Module):
    def __init__(self, RGB_features=None, LWIR_features=None, ch_dim = 512):
        super().__init__()
        self.ch_dim = ch_dim
        self.RGB_features = RGB_features
        ...
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.gmp = nn.AdaptiveMaxPool2d(1)
        self.fc = nn.Linear(ch_dim*2, ch_dim*2, bias= True)
    ...
    def forward(self, RGB_features, LWIR_features):
        ...
        out_RGB = out[:, :self.ch_dim]
        out_lwir = out[:, self.ch_dim:]
        # alpha = out[:,:c].unsqueeze(dim=-1).unsqueeze(dim=-1)
        # beta = out[:,c:].unsqueeze(dim=-1).unsqueeze(dim=-1)
        alpha = out_RGB.unsqueeze(dim=-1).unsqueeze(dim=-1)
        beta = out_lwir.unsqueeze(dim=-1).unsqueeze(dim=-1)
...
class MCFF3(nn.Module):
    ...
    def forward(self, RGB_features, LWIR_features):
        ...
        alpha = out_RGB.unsqueeze(dim=-1).unsqueeze(dim=-1)
        beta = out_lwir.unsqueeze(dim=-1).unsqueeze(dim=-1)
        return alpha*RGB_features + beta*LWIR_features

class MCFF4(nn.Module):
    def __init__(self, RGB_features=None, LWIR_features=None, ch_dim = 512):
        super().__init__()
        self.ch_dim = ch_dim
        self.RGB_features = RGB_features
        ...
        self.gmp = nn.AdaptiveMaxPool2d(1)
        self.fc1 = nn.Linear(ch_dim*2, ch_dim, bias= True)
        self.fc2 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc3 = nn.Linear(ch_dim, ch_dim*2, bias= True)
        nn.init.xavier_uniform_(self.fc1.weight)
        nn.init.xavier_uniform_(self.fc2.weight)
...
class MCFF5(nn.Module):
    ...
    def forward(self, RGB_features, LWIR_features):
        ...
        out_RGB = self.fc1(out)
        out_lwir = self.fc2(out)
        out_RGB = nn.functional.sigmoid(out_RGB)
        out_lwir = nn.functional.sigmoid(out_lwir)
...
class MCFF6(nn.Module):
    def __init__(self, RGB_features=None, LWIR_features=None, ch_dim = 512):
        ...
        self.fc1 = nn.Linear(ch_dim*2, ch_dim, bias= True)
        self.fc2 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc3 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc4 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc5 = nn.Linear(ch_dim, ch_dim*2, bias= True)
...
class MCFF7(nn.Module):
    def __init__(self, RGB_features=None, LWIR_features=None, ch_dim = 512):
        ...
        self.fc3 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc4 = nn.Linear(ch_dim, ch_dim, bias= True)
        self.fc5 = nn.Linear(ch_dim, ch_dim, bias= True)
    ...
    def forward(self, RGB_features, LWIR_features):
        ...
        out = self.gap(out) # torch.Size([2, 1024, 1, 1])
        out = out.view(len(out), -1)

        out = self.fc1(out)
        out = self.fc2(out)
        out = self.fc3(out)
        out_RGB = self.fc4(out)
        out_lwir = self.fc5(out)
...
class VGGBase(nn.Module):
    ...
    def __init__(self):
        ...
        ############################################실험#################################################
        # self.conv5_1_fpn = nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=True)
        # self.conv5_1_bn_fpn = nn.BatchNorm2d(512, affine=True)
        # self.conv5_2_fpn = nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=True)
        # self.conv5_2_bn_fpn = nn.BatchNorm2d(512, affine=True)
        # self.conv5_3_fpn = nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=True)
        # self.conv5_3_bn_fpn = nn.BatchNorm2d(512, affine=True)
        # self.pool5_fpn = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True) 

        # # Replacements for FC6 and FC7 in VGG16
        # self.conv6_1_fpn = nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=True) # atrous convolution
        # self.conv6_1_bn_fpn = nn.BatchNorm2d(512, affine=True)  
        # self.conv6_2_fpn = nn.Conv2d(512, 512, kernel_size=1)

        # self.conv7_1_fpn = nn.Conv2d(512, 256, kernel_size=1, stride=2)
        # self.conv7_2_fpn = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=True)
        # self.conv7_2_bn_fpn = nn.BatchNorm2d(512, affine=True)  

        # self.conv8_1_fpn = nn.Conv2d(512, 256, kernel_size=1, stride=2)
        # nn.init.xavier_uniform_(self.conv8_1_fpn.weight)
        # nn.init.constant_(self.conv8_1_fpn.bias, 0.)
        # self.conv8_2_fpn = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=True)  # stride = 1, by default
        # nn.init.xavier_uniform_(self.conv8_2_fpn.weight)
        # nn.init.constant_(self.conv8_2_fpn.bias, 0.)

        # self.conv9_1_fpn = nn.Conv2d(512, 256, kernel_size=1)
        # nn.init.xavier_uniform_(self.conv9_1_fpn.weight)
        # nn.init.constant_(self.conv9_1_fpn.bias, 0.)
        # self.conv9_2_fpn = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=True)
        # nn.init.xavier_uniform_(self.conv9_2_fpn.weight)
        # nn.init.constant_(self.conv9_2_fpn.bias, 0.)

        # self.conv10_1_fpn = nn.Conv2d(512, 256, kernel_size=1)
        # nn.init.xavier_uniform_(self.conv10_1_fpn.weight)
        # nn.init.constant_(self.conv10_1_fpn.bias, 0.)
        # self.conv10_2_fpn = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=True)
        # nn.init.xavier_uniform_(self.conv10_2_fpn.weight)
        # nn.init.constant_(self.conv10_2_fpn.bias, 0.)
        ...
        # Attention modules that are also used to fuse features of both modalities.
        self.mcff1 = MCFF6(ch_dim = 512)
        self.mcff2 = MCFF6(ch_dim = 512)
        self.mcff3 = MCFF6(ch_dim = 512)
        self.mcff4 = MCFF6(ch_dim = 512)
        self.mcff5 = MCFF6(ch_dim = 512)
        self.mcff6 = MCFF6(ch_dim = 512)
    ...
    def forward(self, image_vis, image_lwir):
        ...
        # skip6_vis = out_vis
        # skip6_lwir = out_lwir
        skip6 = self.mcff2(out_vis, out_lwir)
        #########################################################################

        conv6_feats = torch.cat([out_vis, out_lwir], dim=1)
        # conv6_feats = self.mcff2(out_vis, out_lwir)
        conv6_feats = F.relu(self.feat_2_bn(self.feat_2(conv6_feats)))
        # conv6_feats = F.relu(self.feat_2_bn(conv6_feats))
        conv6_feats = conv6_feats + skip6
        #########################################################################
        ...
        # skip7_vis = out_vis
        # skip7_lwir = out_lwir
        skip7 = self.mcff3(out_vis, out_lwir)
        #########################################################################

        conv7_feats = torch.cat([out_vis, out_lwir], dim=1)
        # conv7_feats = self.mcff3(out_vis, out_lwir)
        # conv7_feats = F.relu(self.feat_3_bn(conv7_feats))
        conv7_feats = F.relu(self.feat_3_bn(self.feat_3(conv7_feats)))
        conv7_feats = conv7_feats + skip7
        ######################################################################### 
        ...
        # skip8_lwir = out_lwir
        # skip8_vis = out_vis
        skip8 = self.mcff4(out_vis, out_lwir)
        #########################################################################

        conv8_feats = torch.cat([out_vis, out_lwir], dim=1)
        # conv8_feats = self.mcff4(out_vis, out_lwir)
        # conv8_feats = F.relu(self.feat_4_bn(conv8_feats))
        conv8_feats = F.relu(self.feat_4_bn(self.feat_4(conv8_feats)))
        conv8_feats = conv8_feats + skip8
        ######################################################################### 
        ...
        # skip9_lwir = out_lwir
        # skip9_vis = out_vis
        skip9 = self.mcff5(out_vis, out_lwir)
        #########################################################################

        conv9_feats = torch.cat([out_vis, out_lwir], dim=1)
        # conv9_feats = self.mcff5(out_vis, out_lwir)
        # conv9_feats = F.relu(self.feat_5_bn(conv9_feats))
        conv9_feats = F.relu(self.feat_5_bn(self.feat_5(conv9_feats)))
        conv9_feats = conv9_feats + skip9
        ######################################################################### 
        ...
        # skip10_lwir = out_lwir
        # skip10_vis = out_vis
        skip10 = self.mcff6(out_vis, out_lwir)
        #########################################################################

        conv10_feats = torch.cat([out_vis, out_lwir], dim=1)
        # conv10_feats = self.mcff6(out_vis, out_lwir)
        # conv10_feats = F.relu(self.feat_6_bn(conv10_feats))
        conv10_feats = F.relu(self.feat_6_bn(self.feat_6(conv10_feats)))
        conv10_feats = conv10_feats + skip10
        ...
        # conv4_3_feats = self.conv5_1_fpn(conv4_3_feats)
        # conv4_3_feats = self.conv5_1_bn_fpn(conv4_3_feats)
        # conv4_3_feats = self.conv5_2_fpn(conv4_3_feats)
        # conv4_3_feats = self.conv5_2_bn_fpn(conv4_3_feats)
        # conv4_3_feats = self.conv5_3_fpn(conv4_3_feats)
        # conv4_3_feats = self.conv5_3_bn_fpn(conv4_3_feats)
        # conv4_3_feats = self.pool5_fpn(conv4_3_feats)
        
        # conv6_feats = self.conv6_1_fpn(conv6_feats)
        # conv6_feats = self.conv6_1_bn_fpn(conv6_feats)
        # conv6_feats = self.conv6_2_fpn(conv6_feats)

        # conv7_feats = self.conv7_1_fpn(conv7_feats)
        # conv7_feats = self.conv7_2_fpn(conv7_feats)
        # conv7_feats = self.conv7_2_bn_fpn(conv7_feats)

        # conv8_feats = self.conv8_1_fpn(conv8_feats)
        # conv8_feats = self.conv8_2_fpn(conv8_feats)

        # conv9_feats = self.conv9_1_fpn(conv9_feats)
        # conv9_feats = self.conv9_2_fpn(conv9_feats)

        # conv10_feats = self.conv10_1_fpn(conv10_feats)
        # conv10_feats = self.conv10_2_fpn(conv10_feats)
...
class SSD300(nn.Module):
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        ...
        for i in range(batch_size):
            ...
            # TODO: Probably changin it to CIoU NMS would help.
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        true_locs = torch.zeros((batch_size, n_priors, 4), dtype=torch.float, device=device) # [1, 41760, 4]
        true_classes = torch.zeros((batch_size, n_priors), dtype=torch.long, device=device) # [1, 41760]
        ...
        #  TODO: I can probably add IoU loss here?
